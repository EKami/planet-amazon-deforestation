...
class TestAmazonKerasClassifier:
    ...
    def test_data_preprocess(self):
        ...
        assert np.array(x_train).shape == (train_files_count, *img_resize, color_channels)
        assert np.array(x_test).shape == (test_files_count, *img_resize, color_channels)
        assert np.array(y_train).shape == (train_files_count, labels_count)
